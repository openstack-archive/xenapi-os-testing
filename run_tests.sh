...
if ! sudo vgs | grep -w $VG >/dev/null; then
    VOL_RESERVE_SIZE_GB=${VOL_RESERVE_SIZE_GB:-4}
    MIN_MEM_BUF_GB=${MIN_MEM_BUF_GB:-2}
    ...
    if [ ${free_mem_GB} -gt $((VOL_RESERVE_SIZE_GB + MIN_MEM_BUF_GB)) ]; then
        LVMNT="/memmnt"
        sudo mkdir -p $LVMNT
        sudo mount -t tmpfs -o size=${VOL_RESERVE_SIZE_GB}G tmpfs $LVMNT
        ...
        if [ "${VOL_USE_SECOND_DISK}" = "True" ]; then
            ...
            VDI=$(run_in_domzero xe vdi-create sr-uuid=$SR name-label="secondDisk" type=user virtual-size=${VOL_RESERVE_SIZE_GB}GiB </dev/null)
